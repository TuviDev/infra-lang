...
- `pip install 'git+https://github.com/TuviDev/infra-lang.git'` fails → ensure Python 3.11+ (`python --version`).
- `infra: command not found` after install → the console-script dir isn't on
  ...
  `python -m infra`.
- LSP not available → install with `pip install 'git+https://github.com/TuviDev/infra-lang.git[lsp]'`.
