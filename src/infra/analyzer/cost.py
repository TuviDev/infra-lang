...
class CostEstimate:
    ...
    def to_markdown(self, currency: str = "USD", factor: float = 1.0) -> str:
        """Render the estimate as a GitHub/GitLab-flavored Markdown table.

        The output is designed to be pasted directly as a PR comment by a
        CI/CD FinOps gate. Amounts are converted with *factor* and labeled
        with *currency*.
        """
        lines: List[str] = [
            "## 💰 Estimated Monthly Infrastructure Cost",
            "",
            f"| Resource | Kind | vCPU | RAM (GB) | Storage (GB) "
            f"| Monthly ({currency}) |",
            "| --- | --- | ---: | ---: | ---: | ---: |",
        ]
        for item in self.items:
            lines.append(
                f"| {item.name} | {item.kind} | {item.vcpu:.2f} "
                f"| {item.ram_gb:.2f} | {item.storage_gb:.2f} "
                f"| {item.monthly_usd * factor:.2f} |"
            )
        lines.append(
            f"| **TOTAL** | | | | "
            f"| **{self.total_monthly_usd * factor:.2f}** |"
        )
        lines += [
            "",
            "> Static order-of-magnitude estimate generated by "
            "[Infra Lang](https://github.com/TuviDev/infra-lang) "
            "(`infra cost`). Not a billing-grade quote.",
            "",
        ]
        return "\n".join(lines)

    def to_html(self, currency: str = "USD", factor: float = 1.0) -> str:
        """Render the estimate as a standalone HTML table.

        All resource names are HTML-escaped. Suitable for embedding in CI
        summaries (e.g. GitHub Actions job summaries) or e-mail reports.
        """
        rows: List[str] = []
        for item in self.items:
            rows.append(
                "    <tr>"
                f"<td>{_html.escape(item.name)}</td>"
                f"<td>{_html.escape(item.kind)}</td>"
                f"<td>{item.vcpu:.2f}</td>"
                f"<td>{item.ram_gb:.2f}</td>"
                f"<td>{item.storage_gb:.2f}</td>"
                f"<td>{item.monthly_usd * factor:.2f}</td>"
                "</tr>"
            )
        rows.append(
            "    <tr>"
            "<td><strong>TOTAL</strong></td><td></td><td></td><td></td><td></td>"
            f"<td><strong>{self.total_monthly_usd * factor:.2f}</strong></td>"
            "</tr>"
        )
        body = "\n".join(rows)
        cur = _html.escape(currency)
        return (
            "<table>\n"
            "  <caption>Estimated Monthly Infrastructure Cost</caption>\n"
            "  <thead>\n"
            "    <tr><th>Resource</th><th>Kind</th><th>vCPU</th>"
            "<th>RAM (GB)</th><th>Storage (GB)</th>"
            f"<th>Monthly ({cur})</th></tr>\n"
            "  </thead>\n"
            "  <tbody>\n"
            f"{body}\n"
            "  </tbody>\n"
