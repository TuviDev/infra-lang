...
- ``secret``    -> Secret
- ``config``    -> ConfigMap
- ``resource``  -> static manifest under ``crds/`` (v0.5.0 plugin system)
...
def _dep_port_map(program: n.Program) -> Dict[str, int]:
    """Map every top-level network-reachable definition to its stable TCP
    port. Used to render ``depends_on`` init-container waits (v0.4.5);
    mirrors the port each definition's Service publishes in the chart.
    """
    ports: Dict[str, int] = {}
    for stmt in program.statements:
        ...
        if isinstance(stmt, n.ServiceDef):
            first = stmt.ports[0] if stmt.ports else None
            ports[name] = (first.host or first.target or 80) if first else 80
        elif isinstance(stmt, n.DatabaseDef):
            ports[name] = _DB_PORT
        elif isinstance(stmt, n.CacheDef):
            ports[name] = _CACHE_PORT
        elif isinstance(stmt, n.QueueDef):
            ports[name] = _QUEUE_PORTS[0]
    return ports
...
class HelmBackend(Backend, BaseYAMLBackend):
        ...
        **kwargs: Any,
        ...
        for stmt in program.statements:
            if isinstance(stmt, n.CustomResourceSpec):
                # v0.5.0 plugin system: the Helm chart carries CRD manifests
                # under crds/, which Helm installs before everything else.
                files[f"{chart}/crds/{_slug(stmt.name)}.yaml"] = self._crd_file(
                    stmt
                )
        ...
        # Defensive: Helm's Go YAML parser rejects a file that starts with a
        # UTF-8 BOM (`yaml: invalid leading UTF-8 octet`). Strip any stray
        # leading \ufeff from every generated file so the chart is byte-clean.
        files = {k: v.lstrip("\ufeff") for k, v in files.items()}

        return CompileResult(files=files, warnings=warnings)
    ...
    def _values_yaml(self, program: n.Program) -> str:
        values: Dict[str, Any] = {}
        dep_ports = _dep_port_map(program)
        for stmt in program.statements:
            ...
            if isinstance(stmt, n.ServiceDef):
                values.setdefault("service", {})[name] = self._service_values(
                    stmt, dep_ports
                )
            elif isinstance(stmt, n.DatabaseDef):
    ...
    def _service_values(
        self,
        node: n.ServiceDef,
        dep_ports: Optional[Dict[str, int]] = None,
    ) -> Dict[str, Any]:
        ...
        # v0.4.5: depends_on -> init-container waits rendered by the
        # Deployment template; only targets with a stable port are listed.
        if node.dependencies and dep_ports:
            deps = [
                {"name": _slug(dep), "port": dep_ports[dep]}
    ...
    def _crd_file(self, node: n.CustomResourceSpec) -> str:
        """Render a ``resource`` custom CRD as a static Helm ``crds/`` file.

        Helm installs everything under ``crds/`` before rendering templates,
        which is exactly the delivery mechanism for plugin CRDs (v0.5.0).
        """
        ...
        for key, value in node.properties:
            if key in ("api_version", "kind", "metadata"):
                continue
            manifest[key] = _crd_value(value)
        return (
...
def _crd_value(value: n.Expression) -> Any:
    """Literal-evaluate a CRD property expression into YAML-ready data."""
    ...
    if isinstance(value, n.Map):
        ...
        for entry in value.entries:
            if isinstance(entry.key, n.Identifier):
                key = entry.key.name
            elif isinstance(entry.key, n.Literal):
                key = str(entry.key.value)
            else:
                key = str(entry.key)
            out[key] = _crd_value(entry.value)
        return out
    if isinstance(value, n.List):
        return [_crd_value(i) for i in value.items]
    if isinstance(value, n.TemplateString):
        return "".join(
            p if isinstance(p, str) else str(_crd_value(p)) for p in value.parts
        )
    return str(value)
...
def _dump_yaml(data: Dict[str, Any]) -> str:
    ...
    out: str = yaml.safe_dump(data, default_flow_style=False, sort_keys=False)
    return out
...
_DEPLOYMENT_TEMPLATE = (
    ...
    "        - name: wait-for-{{ .name }}\n"
    "          image: busybox:1.36\n"
    "          command: [\"sh\", \"-c\", \"until nc -z {{ $.Release.Name }}-"
    "{{ .name }} {{ .port }}; do echo waiting-for-{{ .name }}; sleep 2; done\"]\n"
    "        {{- end }}\n"
    "      {{- end }}\n"
    "      containers:\n"
    "        - name: {{ $name }}\n"
