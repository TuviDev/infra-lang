...
class TestDiagnosticsEnhancement:
    def test_severity_mapping(self):
        ...
        assert any(
            d.severity == DiagnosticSeverity.Error and d.code == "PARSE" for d in diags
        )
        # replicas 0 -> Error (E011)
        diags = mod._diagnose('service api { image: "x" replicas: 0 }', "t.infra")
        assert any(
            d.severity == DiagnosticSeverity.Error and d.code == "E011" for d in diags
        )
        ...
        assert any(
            d.severity == DiagnosticSeverity.Error and d.code == "SEC001" for d in diags
        )
        ...
        assert any(
            d.code == "REL003" and d.severity == DiagnosticSeverity.Warning
            for d in diags
        )
    ...
    def test_related_info_for_duplicate_name(self):
        ...
        assert any(ri.location.range.start.line == 0 for ri in dup.related_information)
...
class TestSignatureHelp:
    def test_service_block_fields(self):
        ...
        assert any("resources" in label for label in labels)
...
class TestFoldingRanges:
    ...
    def test_comment_run_folded(self):
        ...
        ranges = folding_ranges(src)
        assert any(
            r.kind == "comment" and r.start_line == 0 and r.end_line == 2
            for r in ranges
        )
...
class TestSignatureHelpEdgeBranches:
    def test_unbalanced_close_resets_depth(self):
        ...
        assert sh is not None
        labels = [s.label for s in sh.signatures]
        assert any(label.startswith("image") for label in labels)
...
class TestFeatureHandlers:
    ...
    def test_document_highlight_handler(self):
        from lsprotocol.types import DocumentHighlightKind, DocumentHighlightParams
