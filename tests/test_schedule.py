...
class TestScheduleParsing:
    def test_default_only(self):
        svc = _svc(
            'service api { image: "nginx:1.0" schedule { default: replicas 2 } }'
        )
    ...
    def test_with_cron_slot(self):
        svc = _svc(
            'service api { image: "nginx:1.0" schedule { default: replicas 2 "0 9 * * '
            '1-5": replicas 5 } }'
        )
    ...
    def test_multiple_slots(self):
        svc = _svc(
            'service api { image: "nginx:1.0" schedule { default: replicas 1 "0 9 * * '
            '1-5": replicas 5 "0 22 * * *": replicas 1 "0 0 * * 6,7": replicas 1 } }'
        )
        assert len(svc.schedule.slots) == 3

    def test_config_object(self):
        svc = _svc(
            'service api { image: "nginx:1.0" schedule { "0 9 * * 1-5": { replicas: 5, '
...
class TestScheduleKubernetes:
    ...
    def test_cronjob_scales_correct_replicas(self):
        docs = all_k8s_docs(
            'service api { image: "nginx:1.0" schedule { "0 9 * * 1-5": replicas 10 } }'
        )
        cj = next(d for d in docs if d["kind"] == "CronJob")
    ...
    def test_hpa_max_from_peak(self):
        docs = all_k8s_docs(
            'service api { image: "nginx:1.0" schedule { default: replicas 2 "0 9 * * '
            '1-5": replicas 8 "0 20 * * *": replicas 12 } }'
        )
        hpa = next(d for d in docs if d["kind"] == "HorizontalPodAutoscaler")
    ...
    def test_multiple_services_one_rbac(self):
        ...
        sas = [
            ...
            and d["metadata"]["name"] == "infra-scheduler"
        ]
        assert len(sas) == 1
    ...
    def test_output_valid_yaml(self):
        src = (
            'service api { image: "nginx:1.0" schedule { default: replicas 2 "0 9 * * '
            '1-5": replicas 5 "0 22 * * *": replicas 1 } }'
        )
...
class TestScheduleValidation:
    def test_valid_cron_no_error(self):
        r = validate(
            parse(
                'service api { image: "nginx:1.0" schedule { "0 9 * * 1-5": replicas 3 '
                '} }'
            )
        )
    ...
    def test_invalid_cron_error(self):
        r = validate(
            parse(
                'service api { image: "nginx:1.0" schedule { "0 9 * *": replicas 3 } }'
            )
        )
    ...
    def test_zero_replicas_error(self):
        r = validate(
            parse(
                'service api { image: "nginx:1.0" schedule { "0 9 * * 1-5": replicas 0 '
                '} }'
            )
        )
        assert not r.is_valid
